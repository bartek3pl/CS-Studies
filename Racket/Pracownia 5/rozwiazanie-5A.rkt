...
(define (var<? x y) ;zwraca #t kiedy posortowane argumenty
  (symbol<? x y))
...
(define (literal pol x) ;pol = wartościowanie
  (list 'literal pol x))
...
;andmap aplikuje procedurę do każdego elementu
;zwraca #f jeśli jakiś produkuje #f
;wpp zwraca procedurę zaaplikowaną do ostatniego elementu
(define (clause? x)
  (and (tagged-list? 'clause x)  ; '(clause <lista literałów>)
       (andmap literal? (cdr x)))) ;czy każdy element to literał
...
;; oblicza wartość formuły w CNF z częściowym wartościowaniem. 
;; jeśli zmienna nie jest zwartościowana, literał jest uznawany za fałszywy.
(define (valuate-partial val form) ;wartościowanie, formuła
  (define (val-lit l)
...
(define (proof-res x pp pn) ;zmienna znaleziona, 
...
;; klauzulę reprezentujemy jako parę list — osobno wystąpienia pozytywne i negatywne. 
;; Dodatkowo pamiętamy wyprowadzenie tej klauzuli (dowód) i jej rozmiar.
(define (res-clause? x)
...
(define (get-lit test) ; wyciąga literał 
 (car test))

(define (get-lits test) ; wyciąga dalsze literały 
 (cdr test))

;;Współpraca z Sebastian Uryga w zadaniu 1

(define test-a (res-clause (list 'a 'b 'e) (list 'g 'h) 'proof))
(define test-b (res-clause (list 'b 'c 'd) (list 'a 'c) 'proof))


(define (clause-trivial? c)
  (define (helper poz neg)
    (cond
      ((or (null? poz) (null? neg)) #f) ;nastepny literał        
      ((var=? (car poz)
            (car neg)) #t)              
      ((var<? (car poz) (car neg)) (helper (cdr poz) neg))
      ((var<? (car neg) (car poz)) (helper poz (cdr neg)))))
  (helper (res-clause-pos c) (res-clause-neg c))) 

;TESTY
(clause-trivial? test-a)
(clause-trivial? test-b)
  
(define (find xs k)
  (if (eq? (car xs) k)
   (cdr xs)
   (cons (car xs) (find (cdr xs) k))))

(define (resolve c1 c2)
  (define (helper poz neg)
    (cond
      ((or (null? poz) (null? neg)) #f)
      ((var=? (car poz)
              (car neg)) (res-clause
                          (find
                           (remove-duplicates-vars
                            (merge-vars (res-clause-pos c1)
                                        (res-clause-pos c2))) (car poz))
                          (find 
                           (remove-duplicates-vars
                            (merge-vars (res-clause-neg c1)
                                        (res-clause-neg c2))) (car poz)) (proof-res (car poz) c1 c2)))
      ((var<? (car poz) (car neg)) (helper (cdr poz) neg))
      ((var<? (car neg) (car poz)) (helper poz (cdr neg)))))
  
  (if (helper (res-clause-pos c1) (res-clause-neg c2))
      (helper (res-clause-pos c1) (res-clause-neg c2))
      (helper (res-clause-neg c1) (res-clause-pos c2))))

;TESTY
(define test2 (res-clause (list 'b   ) (list 'c 'g 'h) 'proof))
(define test3 (res-clause (list 'a 'c) (list 'd 'k   ) 'proof))
(define test4 (res-clause (list 'd 'f) (list 'g 'h   ) 'proof))

(resolve test2 test3)
(resolve test3 test4)
(resolve test2 test4)

;(proof? (proof-res 'a (proof-axiom (list 'resolve (list 'a 'b) (list 'c 'd) 4))
                      ;(proof-axiom (list 'resolve (list 'e 'f) (list 'a 'h) 4))))
